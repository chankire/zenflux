...
-- 3. Update transaction policy to allow viewers to see aggregated data through the function
-- Remove the overly restrictive policy for viewers
DROP POLICY IF EXISTS "Viewers cannot access individual transactions" ON public.transactions;

-- Create a more nuanced policy that allows function access but not direct queries
CREATE POLICY "Viewers restricted transaction access" 
ON public.transactions 
FOR SELECT 
USING (
  -- Allow org_owners and admins full access
  EXISTS (
    SELECT 1 FROM public.memberships 
    WHERE user_id = auth.uid() 
  ...
  -- Viewers cannot directly query transactions
  -- They must use the get_financial_summary_for_org function
);
