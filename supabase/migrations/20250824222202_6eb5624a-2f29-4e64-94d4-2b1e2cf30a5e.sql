...
-- Bank Accounts - Role-based access
...
CREATE POLICY "Editors can manage bank accounts" 
ON public.bank_accounts 
FOR ALL 
USING (
  EXISTS (
...
-- Viewers can only see aggregated transaction data (no individual transactions)
CREATE POLICY "Viewers can read transaction summaries only" 
ON public.transactions 
FOR SELECT 
USING (
  false -- Viewers cannot see individual transactions, only aggregated data through views/functions
);

-- Balance Snapshots - Role-based access
...
USING (
  EXISTS (
    ...
    AND role IN ('org_owner', 'admin', 'editor')
  )
);

-- Forecast data - Role-based access
